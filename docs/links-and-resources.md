...
First static build status as of 16 May 2026: the repo contains YAML inputs under
`data/`, generated JSON under `docs/data/`, copied Pages assets under `docs/`,
and `.github/workflows/static-data.yml` for scheduled/manual generation.
Local release-prep verification passed for Python/static-builder/frontend syntax
checks, static JSON generation, key JSON validation, README API smoke checks,
and browser checks in both local `/api/*` mode and local static mode. GitHub
Pages still needs to be enabled from the `docs/` folder, the real GitHub Actions
static-data refresh still needs to run from `main`, and the public URL still
needs another-device verification.
