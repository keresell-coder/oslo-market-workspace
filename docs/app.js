...
const refreshStatusConfig = {
  ...
  technical: {
    ...
    check: "Compare latest.csv/report source date, generated timestamp, and dashboard highlights when the RSI14 tab looks stale.",
  },
  fundamentals: {
...
function isStaticDataMode() {
  const params = new URLSearchParams(window.location.search);
  return Boolean(window.OSLO_STATIC_SITE) || params.get("static") === "1" || window.location.hostname.endsWith("github.io");
}

function runtimeLabel() {
  return isStaticDataMode() ? "Static" : "Running";
}
...
async function fetchStaticJson(path) {
  const response = await fetch(staticDataPath(path), { cache: "no-store" });
...
async function staticApi(path, options = {}) {
  ...
  if (method !== "GET") {
    throw new Error("Static GitHub Pages is read-only. Edit the YAML source file through GitHub/Codex and regenerate the site data.");
  }
  ...
  if (url.pathname === "/api/fundamentals") {
    const universe = url.searchParams.get("universe") === "all" ? "all" : "watchlist";
    const data = await fetchStaticJson(`fundamentals-${universe}.json`);
    const symbols = url.searchParams.get("symbols");
    if (!symbols) return data;
    const wanted = new Set(symbols.split(",").map((item) => item.trim().toUpperCase()).filter(Boolean));
    return { ...data, rows: (data.rows || []).filter((row) => wanted.has(String(row.symbol || "").toUpperCase())) };
  }
  if (url.pathname === "/api/consensus") return fetchStaticJson("consensus.json");
  if (url.pathname === "/api/event-monitoring" || url.pathname === "/api/events") return fetchStaticJson("events.json");
  if (url.pathname === "/api/sources") return fetchStaticJson("sources.json");
  if (url.pathname === "/api/benchmarks") {
    const symbol = String(url.searchParams.get("symbol") || "").trim().toUpperCase();
    if (!symbol) throw new Error("Benchmark symbol is required.");
    return fetchStaticJson(`benchmarks/${encodeURIComponent(symbol)}.json`);
  }
  throw new Error(`Static data file is not mapped for ${url.pathname}`);
}
...
function setGlobalLoading(label) {
  ...
  return () => {
    ...
    if (loadingDepth === 0 && health) {
      ...
      health.textContent = runtimeLabel();
    }
  };
}
...
async function api(path, options = {}) {
  if (isStaticDataMode()) return staticApi(path, options);
...
function codexWatchlistPrompt(symbol = "", note = "") {
  const symbolLine = symbol ? `\nTicker to change: ${symbol}` : "";
  const noteLine = note ? `\nCurrent note: ${note}` : "";
  return `Please edit data/watchlist.yml in keresell-coder/oslo-market-workspace.${symbolLine}${noteLine}
...
function staticWatchlistEditMarkup(symbol = "", note = "") {
  ...
  return `
    <div class="static-edit-helper">
      ...
      <button class="secondary" type="button" data-copy-static-prompt>Copy Codex prompt</button>
      <span class="muted" data-static-prompt-status></span>
    </div>
  `;
}

async function copyStaticPrompt(button) {
  const helper = button.closest(".static-edit-helper");
  const prompt = helper?.querySelector("[data-static-watchlist-prompt]")?.value || codexWatchlistPrompt();
  const status = helper?.querySelector("[data-static-prompt-status]");
  try {
...
function setupStaticModeUi(manifest = staticManifest) {
  if (!isStaticDataMode()) return;
  document.body.classList.add("static-data-mode");
  const health = document.getElementById("health");
  if (health) health.textContent = "Static";
  const startPage = document.querySelector(".start-page");
  if (startPage && !startPage.querySelector(".static-mode-banner")) {
    startPage.insertAdjacentHTML("afterbegin", renderStaticModeBanner(manifest));
  }
  const addPanel = document.querySelector(".add-watchlist-panel");
  if (addPanel) {
    addPanel.innerHTML = `
      <summary>Edit watchlist source</summary>
      ${staticWatchlistEditMarkup()}
    `;
  }
  const buttonLabels = {
    "refresh-watchlist": "Reload generated data",
    "refresh-screener-alerts": "Reload RSI14 file",
    "refresh-fundamentals": "Reload generated data",
    "refresh-own-history": "Reload generated data",
    "refresh-technical": "Reload latest generated",
    "refresh-events": "Reload generated digest",
    "refresh-benchmark": "Reload generated benchmark",
    "reload-sources": "Reload source notes",
  };
  Object.entries(buttonLabels).forEach(([id, label]) => {
    const button = document.getElementById(id);
    if (!button) return;
    button.textContent = label;
    button.title = "Reloads the last generated static JSON file. Use the GitHub Actions manual workflow to generate a new dataset.";
  });
}
...
async function loadWatchlist(refresh = false) {
  ...
  setRefreshStatus("watchlist", {
    state: "loading",
    summary: isStaticDataMode()
      ? "Loading the last generated watchlist JSON from GitHub Pages data files."
      : refresh ? "Refreshing watchlist sources: yfinance, RSI14 dashboard, latest.csv, and on-demand NewsWeb." : "Loading watchlist overview and cached source rows.",
  });
  try {
    ...
    const rows = data.rows
      .map((row) => renderWatchlistRow(row))
      .join("");
    ...
    setRefreshStatus("watchlist", {
      ...
      summary: `${data.rows.length} watchlist row${data.rows.length === 1 ? "" : "s"} loaded${isStaticDataMode() ? " from generated static data" : refresh ? " after source refresh" : ""}.`,
      ...
      details: [
        ...
        data.technicalError ? `latest.csv error: ${data.technicalError}` : "latest.csv/report output parsed for technical watchlist rows",
      ],
      errors: [
...
async function handleWatchlistTableClick(event) {
  ...
  const sourcesButton = event.target.closest("[data-open-sources]");
  if (sourcesButton) {
    activateTab("sources");
  }
}
...
function renderWatchlistCompany(row) {
  const note = row.note || "";
  const editBlock = isStaticDataMode()
    ? `
      <details class="compact-details note-details">
        <summary>${note ? "Edit source note" : "Request source note"}</summary>
        ${staticWatchlistEditMarkup(row.symbol, note)}
      </details>
    `
    : `
      <details class="compact-details note-details">
...
function renderWatchlistRow(row) {
  ...
  return `
    <tr class="watchlist-main-row ${alert ? "watchlist-hit" : ""}">
      ...
      <td>${renderWatchlistSourceCell(row)}</td>
      <td>${renderActionsCell(row)}</td>
    </tr>
    <tr class="watchlist-detail-row ${alert ? "watchlist-hit" : ""}">
      <td colspan="9">${renderWatchlistDetailDrawer(row)}</td>
    </tr>
  `;
}
...
function statusToneClass(status) {
  ...
  if (["fresh", "available", "reviewed", "trusted", "source-linked", "generated", "cached"].includes(value)) return "signal-source";
  if (["draft", "gated", "needs review", "unknown", "mixed"].includes(value)) return "signal-draft";
  if (["missing", "stale", "old", "error", "stale-after-error", "stale-error"].includes(value)) return "signal-sell";
  return "signal-neutral";
}

function compactStatusBadge(label, status) {
  const text = label || status || "status n/a";
  return `<span class="signal-badge ${statusToneClass(status || label)}">${escapeHtml(text)}</span>`;
}

function compactFreshnessLabel(value) {
  const status = String(value || "").toLowerCase();
  if (!status) return "unknown";
  if (status === "stale-after-error" || status === "stale-error") return "stale";
...
function compactSourceStatus(value) {
  const status = String(value || "").toLowerCase();
  if (!status) return "n/a";
  if (status === "single-provider-row") return "provider row";
  if (status === "provider-row") return "provider row";
  if (status === "source-linked") return "source linked";
  if (status === "manual-source") return "manual source";
  return value;
}
...
function renderFundamentalHighlight(row) {
  ...
  return `
    <button class="summary-button watchlist-summary" data-open-fundamentals="${escapeHtml(row.symbol)}">
      ...
      <span class="muted">Open Fundamentals</span>
    </button>
  `;
}
...
function renderOwnHistoryWatchlistCell(row) {
  ...
  return `
    <div class="signal-cell watchlist-history-cell">
      <button class="summary-button watchlist-summary compact-summary" data-open-own-history="${escapeHtml(row.symbol)}">
...
function renderPeerContext(row) {
  ...
  const loaded = peer.loadedPeerCount ?? 0;
  const total = peer.peerCount ?? 0;
  const detail = peer.status === "missing"
    ? "Peer context missing"
    : `${loaded}/${total} peer rows loaded`;
  return `
    <button class="summary-button watchlist-summary" data-open-benchmarks="${escapeHtml(row.symbol)}">
      <span class="signal-badge ${peerStatusClass(peer.status)}">${escapeHtml(peer.status || "peer context")}</span>
      <strong>${escapeHtml(peer.groupName || peer.label || "Peer context")}</strong>
...
function renderWatchlistSourceCell(row) {
  const target = row.consensusTargetSummary || {};
  const rating = row.consensusRatingSummary || {};
  const newswebStatus = row.newswebFetch?.status || (row.eventAlert?.count ? "available" : "missing");
  return `
    <div class="watchlist-source-stack">
      ${compactStatusBadge(compactFreshnessLabel(row.cacheStatus || row.priceSummary?.cacheStatus), row.cacheStatus || row.priceSummary?.cacheStatus)}
      ${compactStatusBadge(compactSourceStatus(target.status || "target rows n/a"), target.status)}
      ${compactStatusBadge(newswebStatus, newswebStatus)}
      <span class="muted">${escapeHtml(target.sourceRows ?? rating.providerRows ?? 0)} source row${(target.sourceRows ?? rating.providerRows ?? 0) === 1 ? "" : "s"}</span>
    </div>
  `;
}

function renderWatchlistDetailDrawer(row) {
  const target = row.consensusTargetSummary || {};
  const rating = row.consensusRatingSummary || {};
  const alert = row.screenerSignal;
  const technical = row.technicalSignal;
  const event = row.eventAlert || {};
  const newswebFetch = row.newswebFetch || {};
  return `
    <details class="watchlist-row-detail">
      <summary>Source detail and caveats</summary>
      <div class="watchlist-detail-grid">
        ...
        <section>
          <strong>Provider/source rows</strong>
          <span>Targets: ${escapeHtml(target.sourceRows ?? 0)} row(s), ${escapeHtml(target.status || "status n/a")}, ${escapeHtml(target.confidence || "confidence n/a")}</span>
          <span>Ratings: ${escapeHtml(rating.providerRows ?? 0)} row(s), ${escapeHtml(rating.status || "status n/a")}. Raw labels are not weighted or deduplicated.</span>
        </section>
        <section>
        ...
        <section>
          ...
          <span>${event.count ? `${escapeHtml(event.count)} tracked row(s); latest category ${escapeHtml(event.categoryLabel || "n/a")}` : "No NewsWeb/manual rows found"}</span>
          <span>NewsWeb fetch ${escapeHtml(newswebFetch.status || "unknown")} / ${escapeHtml(shortDate(newswebFetch.fetchedAt || newswebFetch.attemptedAt))}</span>
        </section>
        <section>
          <strong>Navigation</strong>
          <div class="watchlist-detail-actions">
            <button class="link-button" data-open-fundamentals="${escapeHtml(row.symbol)}">Fundamentals</button>
            <button class="link-button" data-open-own-history="${escapeHtml(row.symbol)}">Own history</button>
            <button class="link-button" data-open-technical="${escapeHtml(row.symbol)}">Technical</button>
            <button class="link-button" data-open-benchmarks="${escapeHtml(row.symbol)}">Benchmarks</button>
            <button class="link-button" data-open-events="${escapeHtml(row.symbol)}">News/Events</button>
            <button class="link-button" data-open-sources>Sources</button>
          </div>
        </section>
        <section>
          <strong>Frameworks</strong>
          <span>Gated for a later sprint. No framework score is shown in this Watchlist pass.</span>
        </section>
      </div>
    </details>
  `;
...
function renderActionsCell(row) {
  const editAction = isStaticDataMode()
    ? `<a class="link" href="${escapeHtml(watchlistSourceUrl)}" target="_blank" rel="noreferrer">Edit source</a>`
...
function renderSnapshotHistoryDetails(snapshot) {
    ...
    .map(
      (row) => `
        <tr>
          ...
          <td class="number">${metricValue(row.priceToSalesTrailing12Months, "x")}</td>
          <td class="number">${metricValue(row.enterpriseToRevenue, "x")}</td>
  ...
  return `
    <div class="history-subsection">
      ...
      <div class="mini-table-wrap">
        <table class="mini-table wide-mini-table">
          <thead><tr><th>Date</th><th class="number">TTM P/E</th><th class="number">Fwd P/E</th><th class="number">P/B</th><th class="number">P/S</th><th class="number">EV/revenue</th><th class="number">EV/EBITDA</th><th class="number">Div Yield</th></tr></thead>
          <tbody>${trendRows || `<tr><td colspan="8">No dated snapshots yet.</td></tr>`}</tbody>
        </table>
      </div>
    </div>
  `;
...
function renderEventCell(alert, symbol = "") {
  if (!alert || alert.count === 0) {
    return `
      <button class="summary-button watchlist-summary" data-open-events="${escapeHtml(symbol)}">
        <span class="signal-badge signal-draft">missing</span>
        <strong>No tracked rows</strong>
        <span class="muted">Open News/Events</span>
      </button>
    `;
  }
  return `
    <button class="summary-button watchlist-summary" data-open-events="${escapeHtml(alert.latest?.symbol || symbol || "")}">
      ${compactStatusBadge(alert.level === "high" ? "available" : "available", alert.level === "high" ? "available" : "available")}
      <strong>${escapeHtml(alert.categoryLabel || "category n/a")}</strong>
      <span class="muted">${escapeHtml(alert.count)} tracked row${alert.count === 1 ? "" : "s"}</span>
    </button>
  `;
}
  ...
  if (!item || item.coverageStatus === "missing-from-latest-csv") {
    return `
      <button class="summary-button watchlist-summary compact-summary" data-open-technical="${escapeHtml(row.symbol)}">
        ${compactStatusBadge("missing", "missing")}
        <strong>Missing output</strong>
        <span class="muted">${escapeHtml(item?.coverageDetail || "Open Technical")}</span>
      </button>
    `;
  }
  const dashboard = item.inDashboardScreener ? `<span class="tag dashboard-tag">RSI14 dashboard</span>` : "";
  return `
    <button class="summary-button watchlist-summary compact-summary" data-open-technical="${escapeHtml(row.symbol)}">
      ${compactStatusBadge("source label", "available")}
      <strong>${escapeHtml(item.signal || "label n/a")}; RSI14 ${escapeHtml(formatTechnicalValue(item.rsi14))}</strong>
      <span class="muted">${escapeHtml(item.date || "date n/a")}; risk ${escapeHtml(item.risk || "n/a")}</span>
      ${dashboard}
    </button>
  `;
}
...
async function loadTechnicalIndicators(refresh = false) {
  ...
  try {
    ...
    if (data.screenerError) coverageWarnings.push(`Dashboard highlight unavailable: ${data.screenerError}`);
...
function renderTechnicalSummary(data) {
  ...
  const coverage = data.coverage || {};
  const missingSymbols = coverage.missingSymbols || [];
  target.innerHTML = `
    <div class="alert-card calm">
      <div>
...
function countTechnicalSignals(rows, signal) {
  return rows.filter((row) => row.coverageStatus !== "missing-from-latest-csv" && String(row.signal || "").toLowerCase().includes(signal)).length;
}
      ...
      .map(
        ...
        <tr data-fundamental-symbol="${escapeHtml(row.symbol)}">
          ...
          <td>${renderMetricGroup([
            ...
            ["P/S", value(row.priceToSalesTrailing12Months, "x")],
            ["P/NAV", value(row.pnAv, "x")],
            ["EV/revenue", value(row.enterpriseToRevenue, "x")],
...
function renderFundamentalMetricGuide(guide, validation) {
  ...
  target.innerHTML = `
    <details class="technical-guide fundamental-guide guide-panel">
      ...
      <details class="guide-details">
        <summary>Multiple source-gate review</summary>
        ${renderFundamentalMultipleReview(validation.multipleReview || [])}
      </details>
  ...
  return `
    <tr>
      ...
      <td>
        ...
        <span class="muted">${escapeHtml(item.classification || "")}</span><br>
...
function renderFundamentalMultipleReview(rows) {
  return `
    <div class="table-wrap validation-table">
      <table>
        <thead>
        ...
        <tbody>
          ${
            rows
              .map((row) => {
                const classifications = Object.entries(row.classifications || {})
                  .map(([status, count]) => `${status}: ${count}`)
                  .join("; ");
                const statuses = Object.entries(row.statuses || {})
                  .map(([status, count]) => `${status}: ${count}`)
                  .join("; ");
                const examples = (row.gatedOrMissingRows || [])
                  .map((item) => `${item.symbol}: ${item.status}`)
                  .join("; ");
                return `
                  <tr>
                    <td><strong>${escapeHtml(row.label || row.field)}</strong><br><span class="muted">${escapeHtml(row.field)}</span></td>
                    <td>${escapeHtml(classifications || "n/a")}</td>
                    <td>${escapeHtml(statuses || "n/a")}</td>
                    <td>${escapeHtml(examples || "none")}</td>
                  </tr>
                `;
              })
              .join("") || `<tr><td colspan="4">No multiple-review rows loaded.</td></tr>`
          }
        </tbody>
      </table>
    </div>
...
function renderFundamentalSource(row) {
  const evInputs = row.enterpriseToEbitdaInputs || {};
  const evStatus = row.enterpriseToEbitdaStatus || evInputs.status || "EV/EBITDA status n/a";
  const evValue = row.enterpriseToEbitda === null || row.enterpriseToEbitda === undefined ? "n/a" : value(row.enterpriseToEbitda, "x");
  const providerValue = row.providerEnterpriseToEbitda === null || row.providerEnterpriseToEbitda === undefined ? "n/a" : value(row.providerEnterpriseToEbitda, "x");
  return `
    <div class="fund-cell">
      <span class="tag">${escapeHtml(row.cacheStatus || "cache n/a")}</span>
      ...
      <details class="compact-details">
        ...
        <p class="muted">EV/EBITDA: ${escapeHtml(evValue)}; ${escapeHtml(evStatus)}. Raw provider field: ${escapeHtml(providerValue)}.</p>
        <p class="muted">${escapeHtml(row.enterpriseToEbitdaMethod || "EV/EBITDA is shown only when source inputs pass the app's calculation gate.")}</p>
        <p class="muted">${escapeHtml(row.enterpriseToEbitdaLimitation || "")}</p>
        <p class="muted">Inputs: EV ${escapeHtml(value(evInputs.enterpriseValue))} ${escapeHtml(evInputs.enterpriseValueCurrency || row.currency || "")}; TTM EBITDA ${escapeHtml(value(evInputs.ttmEbitda))} ${escapeHtml(evInputs.ttmEbitdaCurrency || "")}; FX ${escapeHtml(value(evInputs.fxRate))} ${escapeHtml(evInputs.fxPair || "")}.</p>
        ${renderMultipleMetadataDetails(row.multipleSourceMetadata || {})}
      </details>
    </div>
  `;
}

function renderMultipleMetadataDetails(metadata) {
  const fields = [
    "trailingPE",
    "forwardPE",
    "priceToBook",
  ...
  const rows = fields
    ...
    .filter(Boolean)
    .map((item) => {
      const provider = item.providerValue === null || item.providerValue === undefined ? "n/a" : value(item.providerValue, item.unit || "");
      const displayed = item.value === null || item.value === undefined ? "n/a" : value(item.value, item.unit || "");
      return `
        <span class="metric-line">
          <span>${escapeHtml(item.label || item.field)}</span>
          <b>${escapeHtml(displayed)}</b>
        </span>
        <p class="muted">${escapeHtml(item.classification || "classification n/a")}; ${escapeHtml(item.status || "status n/a")}; raw provider ${escapeHtml(provider)}.</p>
        <p class="muted">${escapeHtml(item.method || "")}</p>
      `;
    })
    .join("");
  return rows ? `<div class="metadata-stack">${rows}</div>` : "";
}
...
async function boot() {
  ...
  if (isStaticDataMode()) {
    try {
      ...
      staticManifest = {
        generatedAt: "",
        watchlistEdit: {
          sourceUrl: watchlistSourceUrl,
          workflowUrl: staticWorkflowUrl,
          policy: `Static manifest could not load: ${error.message}`,
        },
      };
    }
    setupStaticModeUi(staticManifest);
  }
  renderStoredRefreshStatuses();
  document.addEventListener("click", async (event) => {
    const copyButton = event.target.closest("[data-copy-static-prompt]");
    if (copyButton) await copyStaticPrompt(copyButton);
  });
  ...
  try {
    ...
    document.getElementById("health").textContent = runtimeLabel();
  } catch {
