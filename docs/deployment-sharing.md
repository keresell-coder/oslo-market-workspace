...
This document records the legacy hosted-backend sharing decision from 06-07 May
2026. It is prep work only; it did not deploy the app externally.

As of 16 May 2026, the preferred public beta direction is no longer this
hosted Python/SQLite path. The project is being remade as a static GitHub Pages
dashboard generated by scheduled/manual GitHub Actions. See
...
## Legacy Public MVP Readiness

The hosted-backend app is close to a public MVP technically, but this path is
paused because it requires more maintenance than desired. If the project returns
to a hosted backend later, complete:
...
checks. This is now legacy backend support. The active low-maintenance path is
the static GitHub Pages remake. The app remains a descriptive, screening-grade
research workspace.
